...
import { useVehicleQuery, useAppointmentsQuery, useUpdateVehicleMutation } from '@/graphql/generated/hooks';
...
export default function VehicleDetailPage() {
  ...
  const [form, setForm] = useState({ make: '', model: '', year: '', vin: '', licensePlate: '', color: '', notes: '', status: 'running', repairStatus: 'none' });
  ...
  useEffect(() => {
    if (vehicle && !editing) {
      setForm({
        ...
        status: vehicle.status ?? 'running', repairStatus: vehicle.repairStatus ?? 'none',
      });
    }
  }, [vehicle, editing]);
  ...
  if (!vehicle) {
    return (
      <><PageHeader title="Vehicle Not Found" description="The vehicle could not be found." /><Card title="Not Found"><Button onClick={() => router.push('/automobile/vehicles')}>Back to Vehicles</Button></Card></>
    );
  }

  return (
    <>
      <PageHeader title={`${vehicle.make} ${vehicle.model}`} description={vehicle.licensePlate ? `License: ${vehicle.licensePlate}` : 'No license plate'} />
      <div className="mb-4">
        <Button variant="outline" onClick={() => router.push('/automobile/vehicles')}>← Back to Vehicles</Button>
      </div>
      ...
      {activeTab === 'repairs' && (
        <Card title="Repair History">
          <AppointmentTable
            ...
            emptyMessage="No repair history found."
            columns={[
              { key: 'scheduledDate', header: 'Date' },
              { key: 'serviceType', header: 'Service' },
              { key: 'assignedMechanic', header: 'Mechanic', render: (a) => <span>{a.assignedMechanic || 'N/A'}</span> },
              { key: 'status', header: 'Status', render: (a) => {
                ...
                return <span className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-medium ${colors[a.status] || 'bg-gray-50 text-gray-500'}`}>{a.status}</span>;
              }},
            ]}
          />
        </Card>
